...
zone_fields = ['ADM1_CODE', 'ADM2_CODE']
#workspace = r"C:\temp"
...
for zone_field in zone_fields:
    ...
    ###make  join
    arcpy.JoinField_management(admin_shapefile, zone_field, nfile_change, zone_field, ["MEAN", "COUNT"])
    ...
    ###make  join
    arcpy.JoinField_management(admin_shapefile, zone_field, nfile_change_negative, zone_field, ["SUM"])

    #calculating the percentage of negative change by admin
    nome_campo = "Neg_ch" + str(indice)
    arcpy.AddField_management(admin_shapefile, nome_campo , "FLOAT","", "", "", "", "NULLABLE")
    ...
    arcpy.CalculateField_management(admin_shapefile, nome_campo,"test( !SUM!, !COUNT!)","PYTHON_9.3", codeblock)
