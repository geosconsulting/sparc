...
def lista_aree_amministrative(paese_ricerca):
    ...
    comando = "SELECT adm0_name,adm2_code,adm2_name FROM sparc_month_prec_norm WHERE adm0_name = '" + paese_ricerca + "';"
...
def normalizzazione_incidenti_mensili_paese(paese_ricerca):
    ...
    comando = "SELECT * FROM sparc_flood_emdat WHERE country = '" + paese_ricerca + "';"
    mesi_inizio = []
    mesi_fine = []
    cur_check.execute(comando);
    for row in cur_check:
